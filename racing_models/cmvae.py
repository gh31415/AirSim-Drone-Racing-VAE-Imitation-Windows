...
from tensorflow.keras.layers import Dense, Flatten, Conv2D, BatchNormalization, Lambda, Concatenate, Conv2DTranspose, \
    Reshape
    ...
    def call(self, x, mode):
        ...
        if mode == 0:
            gate_recon = tf.keras.layers.concatenate(
                [self.p_R(r_params), self.p_Theta(theta_params), self.p_Psi(psi_params), self.p_Phi(phi_params)],
                axis=1)
            img_recon = self.p_img(z)
            return img_recon, gate_recon, means, stddev, z
        elif mode == 1:
