...
#         client.simSpawnObject("gate_" + str(idx), "RedGate16x16", track[idx], 0.75, False, False)
#
#         time.sleep(0.05)
...
def RedGateSpawnerCircle(client, num_gates, radius, radius_noise, height_range, track_offset=[0, 0, 0]):
    track = generate_gate_poses(num_gates=num_gates, race_course_radius=radius, radius_noise=radius_noise, height_range=height_range, direction=0, offset=track_offset)
    # scale = airsim.Vector3r(0.34, 0.17, 0.24)  # square gate
    scale = airsim.Vector3r(0.45, 0.45, 0.45)  # circular gate
    for idx in range(num_gates):
        # client.simSpawnObject("gate_" + str(idx), "gate", track[idx], scale)
        client.simSpawnObject("gate_" + str(idx), "circular_gate", track[idx], scale)
        time.sleep(0.05)
    return track


# # line gate without height_noise
# def LineGateSpawner(client, num_gates, spacing, start_position=[3, 0, 0], direction=[1, 0, 0]):
#     gate_poses = []
#     # scale = airsim.Vector3r(0.3, 0.15, 0.21)  # square gate
#     scale = airsim.Vector3r(0.45, 0.45, 0.45)  # circular gate
#     for idx in range(num_gates):
#         position = [start_position[0] + idx * spacing * direction[0],
#                     start_position[1] + idx * spacing * direction[1],
#                     start_position[2] + idx * spacing * direction[2]]
#         pose = Pose(Vector3r(position[0], position[1], position[2]), Quaternionr(0, 0, 0.707, 0.707))  # 后面的四元数表示将门转向90°
#         # client.simSpawnObject("gate_" + str(idx), "gate", pose, scale)
#         client.simSpawnObject("gate_" + str(idx), "circular_gate", pose,  scale)
#         gate_poses.append(pose)
#         time.sleep(0.05)
#     # return gate_poses


# line gate with height noise
def LineGateSpawner(client, num_gates, spacing, start_position=[3, 0, 0], direction=[1, 0, 0], height_noise=0, lateral_noise=0):
    gate_poses = []
    scale = airsim.Vector3r(0.45, 0.45, 0.45)  # circular gate
    for idx in range(num_gates):
        position = [start_position[0] + idx * spacing * direction[0],
                    start_position[1] + idx * spacing * direction[1],
                    start_position[2] + idx * spacing * direction[2]]
        position[2] += random.uniform(-height_noise, height_noise)  # 增加高度上的随机性
        position[1] += random.uniform(-lateral_noise, lateral_noise)  # Add lateral noise
        pose = Pose(Vector3r(position[0], position[1], position[2]), Quaternionr(0, 0, 0.707, 0.707))
        client.simSpawnObject("gate_" + str(idx), "circular_gate", pose, scale)
        gate_poses.append(pose)
        time.sleep(0.05)
    return gate_poses
...
# s curve
def SGateSpawner(client, num_gates, spacing, start_position=[3, 0, 0], direction=[1, 0, 0], height_noise=0, lateral_noise=0):
    gate_poses = []
    scale = airsim.Vector3r(0.45, 0.45, 0.45)  # circular gate
    for idx in range(num_gates):
        position = [start_position[0] + idx * spacing * direction[0],
        ...
        # 添加S型曲线噪声
        noise_ratio = (idx + 1) / float(num_gates)  # 生成噪声的比例，从0到1逐渐增加
        curve_noise = math.sin(noise_ratio * math.pi)  # 使用sin函数生成S型曲线
        position[1] += curve_noise * lateral_noise  # 在横向位置上应用曲线噪声
