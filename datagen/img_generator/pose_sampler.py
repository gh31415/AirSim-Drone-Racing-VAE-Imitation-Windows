...
import sys
import airsim
from airsimdroneracingvae.types import Pose, Vector3r
import time
import racing_utils
...
class PoseSampler:
    ...
    def configureEnvironment(self):
        # 创建保存图片的文件夹
        images_folder = os.path.join(self.base_path, 'images')
        ...
        for gate_object in self.client.simListSceneObjects(".*[Gg]ate.*"):
            ...
            time.sleep(0.05)
        if self.with_gate:
            scale = airsim.Vector3r(0.45, 0.45, 0.45)
            self.tgt_name = self.client.simSpawnObject("gate_", "circular_gate", Pose(position_val=Vector3r(0, 0, 15)), scale)
